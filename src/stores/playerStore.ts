...
export const usePlayerStore = create<PlayerStore>((set, get) => ({
  ...
  prevTrack: () => {
    ...
    if (!queue.length) return
    const idx = currentTrack ? queue.findIndex(t => t.id === currentTrack.id) : 0
  ...
  removeLocalTrack: id => {
    ...
    set(s => {
      ...
      return {
        ...
        isPlaying: removingCurrent ? newQueue.length > 0 : s.isPlaying,
  ...
  clearLocalTracks: () => {
    const { queue } = get()
    queue.filter(isLocalTrack).forEach(t => revokeEntry(t.id))

    set(s => {
      const nonLocal = s.queue.filter(t => !isLocalTrack(t))
      const currentIsLocal = s.currentTrack !== null && isLocalTrack(s.currentTrack)
      return {
        queue: nonLocal,
        localTrackCount: 0,
        currentTrack: currentIsLocal ? (nonLocal[0] ?? null) : s.currentTrack,
        isPlaying: currentIsLocal ? nonLocal.length > 0 : s.isPlaying,
  ...
  updateLocalTrackDuration: (id, duration) => {
    set(s => ({
      queue: s.queue.map(t => (t.id === id && isLocalTrack(t) ? { ...t, duration } : t)),
      currentTrack:
